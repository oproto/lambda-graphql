    ...
    private static (object? result, System.Collections.Generic.IEnumerable<Diagnostic> diagnostics) ExtractTypeInfoWithDiagnostics(GeneratorSyntaxContext context)
    {
        try
        {
            ...
            var typeInfo = new Models.TypeInfo
            {
                ...
                CSharpClassName = typeSymbol.Name,
                CSharpNamespace = typeSymbol.ContainingNamespace?.IsGlobalNamespace == true
                    ? string.Empty
                    : typeSymbol.ContainingNamespace?.ToDisplayString() ?? string.Empty,
                IsPartial = context.Node is ClassDeclarationSyntax cls &&
...
";
            ...
            // Generate GraphQLFieldMap partial classes for Object types
            foreach (var typeInfo in types)
            {
                if (typeInfo.Kind != Models.TypeKind.Object)
                    continue;

                if (!typeInfo.IsPartial)
                {
                    var diagnostic = Diagnostic.Create(
                        DiagnosticDescriptors.NonPartialGraphQLType,
                        Location.None,
                        typeInfo.CSharpClassName);
                    context.ReportDiagnostic(diagnostic);
                    continue;
                }

                var fieldMapSource = GenerateFieldMapSource(typeInfo);
                if (fieldMapSource != null)
                {
                    context.AddSource($"{typeInfo.CSharpClassName}.GraphQLFieldMap.g.cs", fieldMapSource);
                }
            }
...
";
    ...
    private static string? GenerateFieldMapSource(Models.TypeInfo typeInfo)
    {
        var renamedFields = typeInfo.Fields
            .Where(f => f.Name != f.CSharpPropertyName)
            .ToList();

        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");

        if (!string.IsNullOrEmpty(typeInfo.CSharpNamespace))
        {
            sb.AppendLine();
            sb.AppendLine($"namespace {typeInfo.CSharpNamespace};");
        }

        sb.AppendLine();
        sb.AppendLine($"public partial class {typeInfo.CSharpClassName}");
        sb.AppendLine("{");

        if (renamedFields.Count == 0)
        {
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// Maps GraphQL field names to C# property names for this type.");
            sb.AppendLine("    /// Source-generated from [GraphQLField] attributes.");
            sb.AppendLine("    /// </summary>");
            sb.AppendLine("    public static Oproto.Lambda.GraphQL.Runtime.FieldNameMap GraphQLFieldMap { get; } = Oproto.Lambda.GraphQL.Runtime.FieldNameMap.Identity;");
        }
        else
        {
            sb.AppendLine("    /// <summary>");
            ...
            sb.AppendLine("    /// Source-generated from [GraphQLField] attributes.");
            sb.AppendLine("    /// </summary>");
            sb.Append("    public static Oproto.Lambda.GraphQL.Runtime.FieldNameMap GraphQLFieldMap { get; } = Oproto.Lambda.GraphQL.Runtime.FieldNameMap.Builder()");

            foreach (var field in renamedFields)
            {
                sb.AppendLine();
                sb.Append($"        .Map(\"{EscapeStringLiteral(field.Name)}\", \"{EscapeStringLiteral(field.CSharpPropertyName)}\")");
            }
